...
#       Outputting specific haplotypes      #
#               CSP target                  #
#             December 5, 2019              #
#                K. Sumner                  #
# ----------------------------------------- #
...
for (f in 1:nrow(finalfoo_all)){
  ...
  for (g in 1:nrow(finalfoo_all)) {
    ...
    hapsum = 0
    shared_hap_vecs_output = c()
    for (h in 1:ncol(finalfoo_all)){
      if (finalfoo_all[f,h] != 0 & finalfoo_all[g,h] != 0) {
        haps = 1
        shared_hap_vecs_output = c(shared_hap_vecs_output,names(finalfoo_all[f,])[h])
      } else {
        haps = 0
      }
      hapsum = haps + hapsum
    ...
    shared_hap_vecs_output <- paste0(unique(shared_hap_vecs_output),collapse = ",")
    matrix_add <- c(sid[f],gid[g],hapsum,combined_hap_vecs,shared_hap_vecs_output)
...
# add column names
colnames(edgelist_df) = c("sample_1", "sample_2", "haplotypes_shared", "total_haplotypes","list_haplotypes_shared")
...
edgelist_df_clean = read_csv("Desktop/clean_ids_haplotype_results/CSP/CSP_haplotypes_edgelist_repeated_edges.csv")
...
write.csv(edgelist_df_merge,"Desktop/CSP_haplotypes_edgelist_simplified_number_haps_shared.csv")
