...
# human data sets
hum_monthly_data = readRDS("/Users/kelseysumner/Desktop/Dissertation Materials/SpatialR21 Grant/Final Dissertation Materials/Final Data Sets/De-identified Phase II_v13/clean_data/hum_monthly_data_p3_18AUG2020.RDS")
hum_sick_data = readRDS("/Users/kelseysumner/Desktop/Dissertation Materials/SpatialR21 Grant/Final Dissertation Materials/Final Data Sets/De-identified Phase II_v13/clean_data/hum_sick_data_p3_18AUG2020.RDS")
hum_table_household_data = readRDS("/Users/kelseysumner/Desktop/Dissertation Materials/SpatialR21 Grant/Final Dissertation Materials/Final Data Sets/De-identified Phase II_v13/clean_data/hum_table_data_p3_18AUG2020.RDS")

# look at old table household data sets
hum_table_household_data_old_p1 = readRDS("/Users/kelseysumner/Desktop/Dissertation Materials/SpatialR21 Grant/Final Dissertation Materials/Final Data Sets/Final Cohort data June 2017 to July 2018/Human data/spat21_clean_human_files/hum_table_household_data_19DEC2018.RDS")
hum_table_household_data_old_p2= readRDS("/Users/kelseysumner/Desktop/Dissertation Materials/SpatialR21 Grant/Final Dissertation Materials/Final Data Sets/Cohort data through August July 2019/clean_data/hum_socdem2_data_p2_14NOV2019.rds")
...
table(hum_table_household_data$unq_memID, useNA = "always") # wide format, should have 280 people total (so 28o observations)
...
length(unique(hum_monthly_data$monthly_unq_memID)) # 5619 unique 
...
length(unique(hum_sick_data$sick_unq_memID)) # 1066 unique 
...
colnames(hum_table_household_data_old_p1)
colnames(hum_table_household_data_old_p2)
# need: age and unq_memID
hum_table_household_data_old_p1 = hum_table_household_data_old_p1 %>%
  select(age_y,age_m,age_type,unq_memID)
hum_table_household_data_old_p2 = hum_table_household_data_old_p2 %>%
  select(age,unq_memID)
# merge together by unq_memID and default to p1 age
hum_table_household_data_all = full_join(hum_table_household_data_old_p1,hum_table_household_data_old_p2,by="unq_memID")
...
dups_table = count_table[which(count_table > 1)] # 0 duplicates
# now check overlap with hum_table_household
setdiff(hum_table_household_data$unq_memID,hum_table_household_data_all$unq_memID)
setdiff(hum_table_household_data_all$unq_memID,hum_table_household_data$unq_memID)
names_to_pull_from_new = setdiff(hum_table_household_data$unq_memID,hum_table_household_data_all$unq_memID)
# looks like only s04_9 is needed in the human table household data set, all others are <1 except for M12_9 which doesn't have follow-up data
colnames(hum_table_household_data_all)
hum_table_household_data = hum_table_household_data %>%
  select(unq_memID,age) %>%
  rename(age_all_baseline = age) %>%
  filter(unq_memID %in% names_to_pull_from_new)
hum_table_household_data_all = rbind(hum_table_household_data_all,hum_table_household_data)
hum_table_household_data_all = unique(hum_table_household_data_all)
...
length(unique(hum_monthly_data$unq_memID)) # 268 unique
length(unique(hum_table_household_data_all$unq_memID)) # 287 unique
length(unique(hum_monthly_merged_data$unq_memID)) # 287 unique
...
length(intersect(uniquelist_monthly,uniquelist_household)) # 268 in both
...
# now remove observations that didn't have monthly visit
...
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "S12_2"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "M11_1"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "M11_4"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "S12_3"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "M11_3"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "S12_4"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "M11_2"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "K09_13"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "K13_8"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "K13_9"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "M03_4"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "M12_10"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "M12_9"),]
hum_monthly_merged_data = hum_monthly_merged_data[-which(hum_monthly_merged_data$unq_memID == "S13_4"),]
# look at what ids differ
...
nrow(hum_sick_data) # 1066
nrow(hum_monthly_merged_data) # 5619
nrow(human_merged_all_data) # 6685 (1066+5619=6685)
# looks good
...
sum(test_rs,na.rm=T) # 1066 Rs in sick IDs which is good

# merge together the monthly_unq_memID and sick_unq_memID columns
length(which(is.na(human_merged_all_data$monthly_unq_memID))) # 1066 - all the sick IDs
length(which(is.na(human_merged_all_data$sick_unq_memID))) # 5619 - all the monthly IDs
...
setdiff(sick_ids,monthly_ids) # all sick ids found in monthly data set
setdiff(monthly_ids,sick_ids) # some monthly ids not in sick data set which is fine (N=44)
# looks good

# look one last time at number of participants
length(human_merged_all_data$unq_memID)
length(unique(human_merged_all_data$unq_memID)) # 268 participants
...
write_csv(human_merged_all_data, "Desktop/phase2_hum_monthly_merged_with_table_and_sick_18AUG2020.csv")
write_rds(human_merged_all_data, "Desktop/phase2_hum_monthly_merged_with_table_and_sick_18AUG2020.RDS")
