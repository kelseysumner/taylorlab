...
human_merged_data = read_rds("/Users/kelseysumner/Desktop/Dissertation Materials/SpatialR21 Grant/Final Dissertation Materials/Final Data Sets/De-identified Phase II_v13/merged_data/phase3_spat21_human_merged_all_data_18AUG2020.rds")
...
# 138 variables in final data set
...
sum(str_detect(human_merged_data$sample_name_final,"K")) # 2152
sum(str_detect(human_merged_data$sample_name_final,"M")) # 1980
sum(str_detect(human_merged_data$sample_name_final,"S")) # 2186
# all looks good

# gender
table(human_merged_data$gender_hum_monthly_data, useNA="always")
table(human_merged_data$gender_hum_sick_data, useNA = "always")
# check if there's any case where male and female didn't match
...
length(unique(under1$unq_memID)) # 10
undernames = unique(under1$unq_memID)
# K04_5, K05_7, K07_12, K11_7, M01_8, M06_6, M13_3, S03_5, S06_8, S09_7
...
# 2 months at enrollment, did not age in so remove
# K07_12
k07_12 = human_merged_data[which(human_merged_data$unq_memID == "K07_12"),]
human_merged_data = human_merged_data[-which(human_merged_data$unq_memID == "K07_12"),]
# 1 month at enrollment, did not have enough follow up to age in so remove
# look at K11_7
...
# only 2 months at enrollment and didn't merge in so remove
# M13_3
...
# S06_8
s06_8 = human_merged_data[which(human_merged_data$unq_memID == "S06_8"),]
human_merged_data = human_merged_data[-which(human_merged_data$unq_memID == "S06_8"),]
# 1 month at enrollment, didn't age in so remove
# S09_7
...
new_df = human_merged_data %>%
  ...
  select(unq_memID,age_all_baseline) %>%
  distinct
length(intersect(new_df$unq_memID,unique(human_merged_data$unq_memID))) # 264, correct
...
write_csv(human_merged_data,"Desktop/phase3_spat21_human_merged_data_no_dbs_censoring_18AUG2020.csv")
write_rds(human_merged_data,"Desktop/phase3_spat21_human_merged_data_no_dbs_censoring_18AUG2020.rds")
...
# the 282 dbs that didn't merge in were successfully removed
# write out the cleaned data set with dbs that didn't merge in removed
write_csv(human_merged_data_test,"Desktop/phase3_spat21_human_merged_data_with_dbs_censoring_18AUG2020.csv")
write_rds(human_merged_data_test,"Desktop/phase3_spat21_human_merged_data_with_dbs_censoring_18AUG2020.rds")
