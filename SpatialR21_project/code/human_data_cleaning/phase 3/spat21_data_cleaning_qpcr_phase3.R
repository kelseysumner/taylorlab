...
qpcr_data = read_csv("/Users/kelseysumner/Desktop/Dissertation Materials/SpatialR21 Grant/Final Dissertation Materials/Final Data Sets/De-identified Phase II_v13/raw_data/Mozzie P2 DBS B32-B38 Compiled 03-21-20.csv")
...
m1_df = bind_cols(tidy(model1), confint_tidy(model1)) %>% mutate(model_name="M1: plate32-33") %>% mutate(r_value = summary(model1)$r.squared)
m2_df = bind_cols(tidy(model2), confint_tidy(model2)) %>% mutate(model_name="M2: plate34-35") %>% mutate(r_value = summary(model2)$r.squared)
m3_df = bind_cols(tidy(model3), confint_tidy(model3)) %>% mutate(model_name="M3: plate36-37") %>% mutate(r_value = summary(model3)$r.squared)
m4_df = bind_cols(tidy(model4), confint_tidy(model4)) %>% mutate(model_name="M4: plate38") %>% mutate(r_value = summary(model4)$r.squared)
model_results_df = bind_rows(m1_df,m2_df,m3_df,m4_df)
...
write_csv(model_results_simplified,"Desktop/phase3_spat21_model_results_simplified_human_dbs.csv")
...
model_results = read_csv("/Users/kelseysumner/Desktop/Dissertation Materials/SpatialR21 Grant/Final Dissertation Materials/Final Data Sets/De-identified Phase II_v13/raw_data/phase3_spat21_model_results_simplified_human_dbs.csv")
...
length(which(final_qpcr_merge$HbtubCT1 == "Undetermined" | final_qpcr_merge$HbtubCT1 == "0" | final_qpcr_merge$HbtubCT1 == "0.000")) # 3
length(which(final_qpcr_merge$HbtubCT2 == "Undetermined" | final_qpcr_merge$HbtubCT2 == "0" | final_qpcr_merge$HbtubCT2 == "0.000")) # 1
length(which(final_qpcr_merge$pfr364CT1 == "Undetermined" | final_qpcr_merge$pfr364CT1 == "0" | final_qpcr_merge$pfr364CT1 == "0.000")) # 321
length(which(final_qpcr_merge$pfr364CT2 == "Undetermined" | final_qpcr_merge$pfr364CT2 == "0" | final_qpcr_merge$pfr364CT2 == "0.000")) # 322
...
length(which(is.na(final_qpcr_merge$HbtubCT1))) # 3
length(which(is.na(final_qpcr_merge$HbtubCT2))) # 1
length(which(is.na(final_qpcr_merge$pfr364CT1))) # 321
length(which(is.na(final_qpcr_merge$pfr364CT2))) # 322
...
# export the data set
write_csv(final_qpcr_merge,"Desktop/phase3_spat21_final_qpcr_merge_human_dbs.csv")
...
testdata$`Sample Name`
testdata %>%
  select(`Sample Name`,pfr364CT1,pfr364CT2,HbtubCT1,HbtubCT2,pfr364Q1,pfr364Q2,pfr364Q1_test) %>%
  View()
# looks like there is one missing value but it was correctly calculated
# look to see if any high Pf CT1 value
length(which(final_qpcr_merge$pfr364CT1 > 38)) # 1 with Pf CT1 value greater than 38
...
length(which(final_qpcr_merge$pfr364CT2 > 38)) # 1 with Pf CT2 value greater than 38
...
final_results = read_csv("/Users/kelseysumner/Desktop/Dissertation Materials/SpatialR21 Grant/Final Dissertation Materials/Final Data Sets/De-identified Phase II_v13/clean_data/phase3_spat21_final_qpcr_merge_human_dbs.csv")
...
summary(final_results$pfr364Q1_std) # 321 missing
summary(final_results$pfr364Q2_std) # 322 missing
...
summary(final_results$pfr364Q1_std_censored) # 321 missing
summary(final_results$pfr364Q2_std_censored) # 322 missing
hbcriteria_1$pfr364Q1_std
hbcriteria_2$pfr364Q2_std
# didn't censor any variables
...
summary(final_results$pfr364Q1_std_censored_v2) # 322 missing
summary(final_results$pfr364Q2_std_censored_v2) # 322 missing
# check the output one more time
length(which(final_results$pfr364CT1 >= 38 & is.na(final_results$pfr364CT2))) # 1 observations
length(which(final_results$pfr364CT2 >= 38 & is.na(final_results$pfr364CT1))) # 0 observations
...
# one has all zero Hb CT values: K10-31019-3 at position
test = final_results[which(final_results$`Sample Name` == "K10-31019-3"),]
test$pf_pcr_infection_status
final_results$pf_pcr_infection_status[which(final_results$`Sample Name` == "K10-31019-3" & final_results$`Well Position` == "M11")] = NA
table(final_results$pf_pcr_infection_status, useNA = "always")
...
# K10-31019-3 - both negative but first failed to amplify Hb so delete first entry
dup_data$pf_pcr_infection_status[which(dup_data$`Sample Name`=="K10-31019-3")]
qpcr_data = qpcr_data[-which(qpcr_data$`Well Position` == "M11" & qpcr_data$`Sample Name` == "K10-31019-3"),]
# S09-21112019-1 - both negative, go with first entry
dup_data$pf_pcr_infection_status[which(dup_data$`Sample Name`=="S09-21112019-1")]
qpcr_data = qpcr_data[-which(qpcr_data$`Well Position` == "B9" & qpcr_data$`Sample Name` == "S09-21112019-1"),]
...
length(which(is.na(qpcr_data$HbtubCT1) & is.na(qpcr_data$HbtubCT2))) # we had 0 where both the Hbtub values were missing
...
qpcr_data$clean_sample_id = clean_sample_id 
qpcr_data %>%
...
length(which(is.na(qpcr_data$HbtubCT2))) # 2
length(which(is.na(qpcr_data$pfr364CT1))) # 319
length(which(is.na(qpcr_data$pfr364CT2))) # 320
# still looks good
...
# pfr364R_
summary(qpcr_data$`pfr364R²`)
str(qpcr_data$`pfr364R²`)
# look good, clean
# no longer need because have standardized one
qpcr_data$`pfr364R²` <- NULL
...
write_csv(qpcr_data, "Desktop/phase3_spat21_qpcr_data_clean_human_dbs_18AUG2020.csv")
write_rds(qpcr_data, "Desktop/phase3_spat21_qpcr_data_clean_human_dbs_18AUG2020.RDS")
