...
qpcr_data = read_csv("Desktop/Dissertation Materials/Turkana Project/EMBATALK/Lab materials/Original data/Round 3/EMBATALK DBS compiled 10May2020.csv")
...
length(unique(qpcr_data$`Sample Name`)) # 1598 unique 
...
write_csv(qpcr_dup_df,"Desktop/turkana_qpcr_duplicates_21MAY2020.csv")
...
merged_data %>%
  ...
  View()
length(which(is.na(merged_data$`Well Position`))) # 5768-1588=4180, good 
...
summary(merged_data$pfr364Q1_std_censored) # 4202 missing
summary(merged_data$pfr364Q2_std_censored) # 4185 missing
...
summary(merged_data$pfr364Q1_std_censored_v2) # 5253 missing
summary(merged_data$pfr364Q2_std_censored_v2) # 5250 missing
...
# looks like we had 1 of these
merged_data$pf_pcr_infection_status[merged_data$`sample ID`=="S0209"] = NA
# check the change
table(merged_data$pf_pcr_infection_status, useNA = "always")
# looks good
...
write_csv(positive_data,"Desktop/EMBATALK_positive_samples_round3_21MAY2020.csv")
...
length(setdiff(positive_data$`sample ID`,positive_data_round1$`sample ID`)) # 518, good
...
length(setdiff(positive_data$`sample ID`,positive_data_round2$`sample ID`)) # 518, good
...
merged_data_round1 = read_rds("Desktop/Dissertation Materials/Turkana Project/EMBATALK/Lab materials/Merged data/All merged pcr results - round 2/EMBATALK_inventory_with_qpcr_14FEB2020.RDS")
...
merged_data_round1_have = merged_data_round1[-which(is.na(merged_data_round1$`pfr364R²`)),] # 4179
merged_data_round2_have = merged_data[-which(is.na(merged_data$`pfr364R²`)),] # 1588
...
table(merged_data_all$pf_pcr_infection_status, useNA = "always") # 2546 positive
table(merged_data_round1_have$pf_pcr_infection_status, useNA = "always") # 2028
table(merged_data_round2_have$pf_pcr_infection_status, useNA = "always") # 518

# export the data set as CSV and RDS files
write_csv(merged_data_all, "Desktop/EMBATALK_inventory_with_qpcr_21MAY2020.csv")
write_rds(merged_data_all, "Desktop/EMBATALK_inventory_with_qpcr_21MAY2020.RDS")
