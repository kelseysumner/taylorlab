...
qpcr_data = read_csv("Desktop/Dissertation Materials/Turkana Project/EMBATALK/Lab materials/Original data/Round 4/EMBATALK DBS compiled 2July2020.csv")

# read in the lab inventory of samples received and punched
inventory_data = read_csv("Desktop/Dissertation Materials/Turkana Project/EMBATALK/Lab materials/Original data/Round 4//EMBATALK Database.csv")
...
dups_table
dups_out_data = inventory_data[which(inventory_data$`sample ID` %in% names(dups_table)),]
write_csv(dups_out_data,"Desktop/embatalk_inventory_duplicates_7JUL2020.csv")
...
length(unique(qpcr_data$`Sample Name`)) # 1163 unique 
...
merged_data %>%
  ...
  View()
length(which(is.na(merged_data$`Well Position`))) # 6822-1151=5671, good 
setdiff(qpcr_data$`sample ID`,inventory_data$`sample ID`)
# looks good

# check for duplicates in the merged data just in case
length(unique(merged_data$`sample ID`)) # 6822 unique 
...
summary(merged_data$pfr364Q1) # 4180 missing
summary(merged_data$pfr364Q2) # 4180 missing
...
summary(merged_data$pfr364Q1_std_censored) # 5678 missing
summary(merged_data$pfr364Q2_std_censored) # 5672 missing
...
summary(merged_data$pfr364Q1_std_censored) # 6460
summary(merged_data$pfr364Q2_std_censored) # 6473
...
summary(merged_data$pfr364Q1_std_censored_v2) # 6461 missing
summary(merged_data$pfr364Q2_std_censored_v2) # 6475 missing
...
write_csv(positive_data,"Desktop/EMBATALK_positive_samples_round3_7JUL2020.csv")
...
length(setdiff(positive_data$`sample ID`,positive_data_round1$`sample ID`)) # 400, good
length(setdiff(positive_data_round1$`sample ID`,positive_data$`sample ID`)) # 661, good
intersect(positive_data$`sample ID`,positive_data_round2$`sample ID`) # no overlap, good
length(setdiff(positive_data$`sample ID`,positive_data_round2$`sample ID`)) # 400, good
...
length(setdiff(positive_data$`sample ID`,positive_data_round3$`sample ID`)) # 400, good
...
merged_data_round1 = read_rds("Desktop/Dissertation Materials/Turkana Project/EMBATALK/Lab materials/Merged data/All merged pcr results - round 3/EMBATALK_inventory_with_qpcr_21MAY2020.RDS")
...
merged_data_round1_have = merged_data_round1[-which(is.na(merged_data_round1$`pfr364R²`)),] # 5767
merged_data_round2_have = merged_data[-which(is.na(merged_data$`pfr364R²`)),] # 1151
merged_data_missing = merged_data[-which(merged_data$`sample ID` %in% merged_data_round1_have$`sample ID` | 
                                           merged_data$`sample ID`%in% merged_data_round2_have$`sample ID`),] # 1: called SPOILT so guess spoiled?
# total in inventory: 6822
# 5767+1151+1=6919 - maybe some duplicates? but doesn't look like it
...
length(unique(merged_data_all$`sample ID`)) # 6919 unique 
...
table(merged_data_all$pf_pcr_infection_status, useNA = "always") # 2946 positive
table(merged_data_round1_have$pf_pcr_infection_status, useNA = "always") # 2546
table(merged_data_round2_have$pf_pcr_infection_status, useNA = "always") # 400

# export the data set as CSV and RDS files
write_csv(merged_data_all, "Desktop/EMBATALK_inventory_with_qpcr_7JUL2020.csv")
write_rds(merged_data_all, "Desktop/EMBATALK_inventory_with_qpcr_7JUL2020.RDS")
