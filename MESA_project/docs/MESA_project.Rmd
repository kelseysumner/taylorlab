...
## Haplotype Phylogenetic Analyses and Pouplation Genetics [Cody]
...
* Standardize Ct values for each sample to calculate the parasites/uL in each sample
    ...
    2. *Censoring criteria for parasitemia:*
        - *Samples with Hb CT values < 35*
        - *Samples with Pf CT values < 38*
    3. *Indicated sample was positive for Pf if at least one Pf CT values was < 38 and met Hb CT value criteria (pf_pcr_infection_status)*
    4. *Standardized and combined parasitemia (pfr364Q_std_combined) as follows:*
        - *If 1 duplicate Pf positive, used that duplicates standardized parasitemia for the participant*
        - *If both duplicates Pf positive, averaged the duplicates standardized parasitemia for the participant*
